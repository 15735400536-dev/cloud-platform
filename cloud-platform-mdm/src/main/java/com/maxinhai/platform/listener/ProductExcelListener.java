...
@Slf4j
@Component
public class ProductExcelListener implements ReadListener<ProductExcelBO> {
    ...
    // 存储读取到的数据
    private final List<ProductExcelBO> dataList = new ArrayList<>(BATCH_COUNT);

    /**
     * 每读取一行数据就会调用该方法
     */
    @Override
    public void invoke(ProductExcelBO data, AnalysisContext context) {
        log.info("读取到数据: {}", data);
        dataList.add(data);
    ...
    private void saveData() {
        ...
        // 保存数据
        productService.saveExcelData(dataList);
        log.info("数据保存完成！");
    }
