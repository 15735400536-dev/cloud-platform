...
//    @Scheduled(initialDelay = 5000, fixedRate = 120000)
    ...
    public List<TaskOrder> queryCanStartWorkTaskList() {
        DateTime today = DateUtil.parseDate("2019-01-01");
        Date beginTime = DateUtil.beginOfDay(today).toJdkDate();
        Date endTime = DateUtil.beginOfDay(today).toJdkDate();
        return taskOrderMapper.queryCanStartedTaskList(beginTime, endTime);
    }
